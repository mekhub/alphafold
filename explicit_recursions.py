...
def update_Z( i, j, N, sequence, ligated, all_ligated, \
    ...
    if (( sequence[i] == 'C' and sequence[j] == 'G' ) or ( sequence[i] == 'G' and sequence[j] == 'C' )) and \
        ...
        if (ligated.array[ i ]) and (ligated.array[ (j-1) % N]):
            ...
            Z_BP[i][j].contrib.append( [ (1.0/Kd_BP ) * ( C_eff[(i+1) % N][(j-1) % N].Q * l * l * l_BP), [[C_eff,i+1,j-1]]] )
    ...
    if ligated.array[(j-1) % N]:
        ...
        C_eff[i][j].contrib.append( [C_eff[i][(j-1) % N].Q * l, [[C_eff, i, j-1]]] )

    C_eff[i][j].Q += C_init * Z_BP[i][j].Q * l_BP
    C_eff[i][j].contrib.append( [ C_init * Z_BP[i][j].Q * l_BP, [[Z_BP, i, j]]] )
    ...
    if ligated.array[(j-1) % N]:
        ...
        Z_linear[i][j].contrib.append( [ Z_linear[i][(j - 1) % N].Q, [[Z_linear, i, j-1]]]  )

    Z_linear[i][j].Q += Z_BP[i][j].Q
    Z_linear[i][j].contrib.append( [ Z_BP[i][j].Q, [ [Z_BP, i, j]]] )
...
def get_Z_final(N, sequence, ligated, all_ligated, \
    ...
    for i in range( N ):
        ...
        if not ligated.array[(i + N - 1) % N]:
            ...
            Z_final[i].contrib.append( [ Z_linear[i][(i-1) % N].Q, [[Z_linear, i, i-1 ]] ] )
        else:
            # Scaling Z_final by Kd_lig/C_std to match previous literature conventions
            Z_final[i].Q += C_eff[i][(i - 1) % N].Q * l / C_std
            Z_final[i].contrib.append( [ C_eff[i][(i - 1) % N].Q * l / C_std, [[C_eff, i, i-1]] ] )
